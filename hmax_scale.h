...
FIND_LAYER_Y_NEAREST(z, yWidth, vy1, vy2, y1, y2);
FIND_LAYER_X_NEAREST(z, xWidth, vx1, vx2, x1, x2);
vy2 = min(vy2, PY_COUNT - 1);
vx2 = min(vx2, PX_COUNT - 1);
if ((2 * (vy2 - vy1 + 1) < yWidth) || (2 * (vx2 - vx1 + 1) < xWidth)) {
    // If more than halfway off the image edge in either dimension, quit.
...
float yFiltStart = (LAYER_Y_CENTER(z, y1) - THIS_Y_CENTER) / pySpace * yFiltSpace;
float xFiltStart = (LAYER_X_CENTER(z, x1) - THIS_X_CENTER) / pxSpace * xFiltSpace;
